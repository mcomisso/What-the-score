...
public class Game {
    ...
    @Relationship(deleteRule: .nullify) public var teams: [Team]
    @Relationship(deleteRule: .nullify) public var intervals: [Interval]

    public init(date: Date = .now, teams: [Team], intervals: [Interval]) {
        self.date = date
        self.teams = teams
...
public extension Array where Element == Score {
    var totalScore: Int {
        map(\.value).reduce(0, +)
    }
